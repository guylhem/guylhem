...
static int			iff_port_v4 = -1;
static int			iff_port_v6 = -1;
...
static void init(void)
{
	...
	x = getenv("IFF_PORT_V4");
	if (x != NULL) {
		iff_port_v4 = strtol(x, NULL, 10);
		my_syslog(LOG_INFO, "force_bind: Only when IPV4 port %d.\n",
			iff_port_v4);
	}

	x = getenv("IFF_PORT_V6");
	if (x != NULL) {
		iff_port_v6 = strtol(x, NULL, 10);
		my_syslog(LOG_INFO, "force_bind: Only when IPV6 port %d.\n",
			iff_port_v6);
	}
...
static void alter_sa(const int sockfd, struct sockaddr *sa)
{
	...
	case AF_INET:
		...
		wanted = ntohs(sa4->sin_port);
		if (iff_port_v4>0) {
			if (iff_port_v4 != wanted) {
				matched=0;
			}
		}
		break;
	...
	case AF_INET6:
		...
		wanted = ntohs(sa6->sin6_port);
		if (iff_port_v6>0) {
			if (iff_port_v6 != wanted) {
				matched=0;
			}
		}
		break;
	...
	my_syslog(LOG_INFO, "force_bind: wanted %s#%d\n", 
	 addressBuffer, wanted);

	if (matched == 0) {
	my_syslog(LOG_INFO, "force_bind: unmatched request\n");
 	} else {
...
static void bw(const int sockfd, const ssize_t bytes)
{
	...
	printf("diff_ms=%lld rest=%llu bytes=%u allowed=%llub\n",
		diff_ms, q->rest, (unsigned int) bytes, allowed);
